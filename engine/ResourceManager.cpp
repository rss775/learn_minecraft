...
std::vector<std::shared_ptr<Mesh>> ResourceManager::loadObjects(const std::string &filename) {
    ...
    while (!file.eof()) {
        ...
        if (line[0] == 'm') {
            ...
            maters.insert({matName, sf::Color(color[0], color[1], color[2])});
        }
        if (line[0] == 'v') {
            // TODO: implement (lesson 3)
        ...
        if (line[0] == 'o') {
            // TODO: implement (lesson 3)
            if (!tris.empty()) {
                objects.emplace_back(std::make_shared<Mesh>(ObjectNameTag(filename + "_temp_obj_" 
                + std::to_string(objects.size())), tris));
                tris.clear();
            }
        }
    }
