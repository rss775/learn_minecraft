...
std::vector<Triangle> Camera::project(std::shared_ptr<Mesh> mesh) {
    ...
    for (auto &t : mesh->triangles()) {
        ...
        double dot = modelTriangle.norm().dot((position() 
            - Vec3D(modelTriangle[0])).normalized());

        if (dot < 0) {
            continue;
        }

        Triangle modelViewTtiangle = modelTriangle * V;

        clippedTriangles.clear();
        tempBuffer.clear();

        clippedTriangles.emplace_back(modelViewTtiangle);
